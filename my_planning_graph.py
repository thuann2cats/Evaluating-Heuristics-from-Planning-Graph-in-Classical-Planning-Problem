...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        actionB_negated_effects = set(~l for l in actionB.effects)
        return bool(actionB_negated_effects & actionA.effects)
    ...
    def _interference(self, actionA, actionB):
        ...
        # raise NotImplementedError
        actionB_negated_effects = set(~l for l in actionB.effects)
        actionB_effects_negate_actionA_preconditions = bool(actionA.preconditions & actionB_negated_effects)

        actionA_negated_effects = set(~l for l in actionA.effects)
        actionA_effects_negate_actionB_preconditions = bool(actionB.preconditions & actionA_negated_effects)

        return actionA_effects_negate_actionB_preconditions or actionB_effects_negate_actionA_preconditions
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for preA in actionA.preconditions:
            for preB in actionB.preconditions:
                conflict |= self.parent_layer.is_mutex(preA, preB)
        return conflict
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        all_pairwise_mutex = True
        for prev_action_A in self.parents[literalA]:
            for prev_action_B in self.parents[literalB]:
                all_pairwise_mutex &= self.parent_layer.is_mutex(prev_action_A, prev_action_B)
        return all_pairwise_mutex

    def _negation(self, literalA, literalB):
        """ Return True if two literals are negations of each other """
        # TODO: implement this function
        # raise NotImplementedError
        return (((literalA.op == '~') and (literalB.op != '~') and (literalA.args[0] == literalB))
                or
                ((literalB.op == '~') and (literalA.op != '~') and (literalB.args[0] == literalA)))
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        goals_to_look_for = set(self.goal)
        level_cost_dict = dict((goal, None) for goal in self.goal)
        num_goals_found = 0

        for literal_layer_index, literal_layer in enumerate(self.literal_layers):
            for goal in goals_to_look_for:
                if goal in literal_layer:
                    if level_cost_dict[goal] is None:
                        level_cost_dict[goal] = literal_layer_index
                        num_goals_found += 1
            if len(goals_to_look_for) == num_goals_found:
                break

        if num_goals_found < len(goals_to_look_for):
            curr_layer_index = len(self.literal_layers) - 1
            while num_goals_found < len(goals_to_look_for):
                self._extend()
                curr_layer_index += 1
                for goal in goals_to_look_for:
                    if goal in self.literal_layers[-1]:
                        if level_cost_dict[goal] is None:
                            level_cost_dict[goal] = curr_layer_index
                            num_goals_found += 1
                if self._is_leveled:
                    for goal in goals_to_look_for:
                        if level_cost_dict[goal] is None:
                            level_cost_dict[goal] = float("inf")
                            num_goals_found += 1

        assert num_goals_found == len(goals_to_look_for)
        return sum(level_cost_dict.values())
    ...
    def h_maxlevel(self):
        ...
        # raise NotImplementedError
        max_level_found = None
        for literal_layer_index, literal_layer in enumerate(self.literal_layers):
            if self.goal <= literal_layer:
                max_level_found = literal_layer_index
                break
        if max_level_found is None:
            curr_layer_index = len(self.literal_layers) - 1
            while max_level_found is None:
                self._extend()
                curr_layer_index += 1
                if self.goal <= self.literal_layers[-1]:
                    max_level_found = curr_layer_index
                if self._is_leveled:
                    max_level_found = float("inf")
    ...
    def h_setlevel(self):
        ...
        assert max_level_found is not None
        return max_level_found
